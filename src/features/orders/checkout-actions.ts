...
export async function verifyAndFinalizeOrder(input: {
  ...
  // Use admin client for cross-table fulfilment writes (idempotent on payment_id).
  ...
  await admin.from("orders").update({ status: "paid" }).eq("id", input.orderId);
  await admin.from("order_status_history").insert({ order_id: input.orderId, status: "paid", note: "Payment captured" });
  ...
  const { data: ord } = await admin
    .from("orders")
    .select("coupon_id, discount, buyer_id")
    .eq("id", input.orderId)
    .single();
  if (ord?.coupon_id) {
    ...
    await admin.rpc("increment_coupon_use", { p_coupon: ord.coupon_id });
  }
  ...
  for (const sellerId of sellers) {
    const { data: shipment } = await admin
      .from("shipments")
      .insert({ order_id: input.orderId, seller_id: sellerId, status: "pending" })
      .select("id")
      .single();
    if (shipment) {
