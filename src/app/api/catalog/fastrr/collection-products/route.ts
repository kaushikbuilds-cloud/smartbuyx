...
export async function GET(req: NextRequest) {
  ...
  const { data: products, count } = await admin
    .from("products")
    .select("id, title, description, brand, images, status, updated_at, weight_kg, categories(name), product_variants(id, sku, price, options, updated_at, inventory(quantity))", { count: "exact" })
    .eq("status", "active")
